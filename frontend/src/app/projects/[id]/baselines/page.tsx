            ...
            {selected.requirements?.length > 0 && (
              <div className="overflow-hidden rounded-lg border border-astra-border">
                {selected.requirements.map((r: any, i: number) => {
                  const lvl = (r.level_snapshot || 'L1') as RequirementLevel;
                  return (
                    <div key={i} className="flex items-center gap-3 border-b border-astra-border/50 px-4 py-2 last:border-0">
                      ...
                      <span className="font-mono text-xs text-blue-400">{r.req_id_snapshot}</span>
                      <span className="flex-1 truncate text-xs text-slate-300">{r.title_snapshot}</span>
                    </div>
                  );
                })}
              </div>
          ...
          <div className="rounded-xl border border-astra-border bg-astra-surface p-5">
            ...
            <div className="grid grid-cols-3 gap-4 mb-4">
              <div className="rounded-lg border border-emerald-500/20 bg-emerald-500/5 p-3 text-center">
                ...
                <div className="text-lg font-bold text-emerald-400">{compareResult.added?.length || 0}</div>
                <div className="text-[10px] text-slate-500">Added</div>
              </div>
              <div className="rounded-lg border border-red-500/20 bg-red-500/5 p-3 text-center">
                <Minus className="mx-auto h-4 w-4 text-red-400 mb-1" />
                <div className="text-lg font-bold text-red-400">{compareResult.removed?.length || 0}</div>
                <div className="text-[10px] text-slate-500">Removed</div>
              </div>
              <div className="rounded-lg border border-amber-500/20 bg-amber-500/5 p-3 text-center">
                <Edit3 className="mx-auto h-4 w-4 text-amber-400 mb-1" />
                <div className="text-lg font-bold text-amber-400">{compareResult.changed?.length || 0}</div>
                <div className="text-[10px] text-slate-500">Changed</div>
              </div>
            </div>
            {compareResult.added?.length > 0 && <div className="mb-3"><h4 className="text-[10px] font-semibold uppercase text-emerald-400 mb-1">Added</h4>{compareResult.added.map((r: any, i: number) => <div key={i} className="text-xs text-slate-300 py-0.5"><span className="font-mono text-blue-400">{r.req_id || r.req_id_snapshot}</span> {r.title || r.title_snapshot}</div>)}</div>}
            {compareResult.removed?.length > 0 && <div className="mb-3"><h4 className="text-[10px] font-semibold uppercase text-red-400 mb-1">Removed</h4>{compareResult.removed.map((r: any, i: number) => <div key={i} className="text-xs text-slate-300 py-0.5 line-through opacity-60"><span className="font-mono text-blue-400">{r.req_id || r.req_id_snapshot}</span> {r.title || r.title_snapshot}</div>)}</div>}
            {compareResult.changed?.length > 0 && <div><h4 className="text-[10px] font-semibold uppercase text-amber-400 mb-1">Changed</h4>{compareResult.changed.map((r: any, i: number) => <div key={i} className="text-xs text-slate-300 py-0.5"><span className="font-mono text-blue-400">{r.req_id}</span> {r.title} <span className="text-[10px] text-amber-400">{r.changes?.join(', ')}</span></div>)}</div>}
          </div>
        ) : null
      )}
      ...
      {showCreate && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm">
          <div className="w-full max-w-md rounded-2xl border border-astra-border bg-astra-surface p-6 shadow-2xl">
            ...
            <div className="space-y-3">
              <div><label className="mb-1 block text-[10px] font-semibold uppercase tracking-wider text-slate-500">Name</label>
                <input value={newName} onChange={(e) => setNewName(e.target.value)} placeholder="e.g., PDR Baseline v2.0" className="w-full rounded-lg border border-astra-border bg-astra-surface-alt px-3 py-2 text-sm text-slate-200 outline-none focus:border-blue-500/50" /></div>
              <div><label className="mb-1 block text-[10px] font-semibold uppercase tracking-wider text-slate-500">Description</label>
                <textarea value={newDesc} onChange={(e) => setNewDesc(e.target.value)} rows={2} className="w-full rounded-lg border border-astra-border bg-astra-surface-alt px-3 py-2 text-sm text-slate-200 outline-none focus:border-blue-500/50" /></div>
            </div>
            <div className="mt-4 flex justify-between">
              <button onClick={() => setShowCreate(false)} className="rounded-lg border border-astra-border px-4 py-2 text-xs text-slate-400">Cancel</button>
              <button onClick={handleCreate} disabled={!newName.trim() || creating} className="rounded-lg bg-blue-500 px-4 py-2 text-xs font-semibold text-white hover:bg-blue-600 disabled:opacity-50">{creating ? 'Creating…' : 'Create'}</button>
            </div>
          </div>
        </div>
      )}

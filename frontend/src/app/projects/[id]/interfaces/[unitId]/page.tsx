...
export default function UnitDetailPage() {
  ...
  const [connForm, setConnForm] = useState({ designator: '', name: '', connector_type: 'd_sub_25', gender: 'male_pin', total_contacts: 25 });
  ...
  const handleAddConnector = async () => {
    ...
    try {
      ...
      setConnForm({ designator: '', name: '', connector_type: 'd_sub_25', gender: 'male_pin', total_contacts: 25 });
      fetchUnit();
    } catch (e: any) { alert(e.response?.data?.detail || 'Failed to create connector'); }
    setSavingConn(false);
  };
  ...
  const handleAddBus = async () => {
    setSavingBus(true);
    try {
      await interfaceAPI.createBus({ unit_id: unitId, project_id: projectId, ...busForm });
      setShowAddBus(false);
      setBusForm({ name: '', bus_type: 'mil_std_1553b', protocol_version: '' });
      fetchUnit();
    } catch (e: any) { alert(e.response?.data?.detail || 'Failed to create bus'); }
    setSavingBus(false);
  };
    ...
    <div>
      ...
      {tab === 'connectors' && (
        <div className="space-y-3">
          <div className="flex items-center justify-between mb-2">
            ...
            <button onClick={() => setShowAddConnector(true)} className="flex items-center gap-1.5 rounded-lg bg-blue-500 px-3 py-2 text-xs font-semibold text-white hover:bg-blue-600">
              <Plus className="h-3.5 w-3.5" /> Add Connector
            </button>
          </div>
          {showAddConnector && (
            <div className="rounded-xl border border-blue-500/30 bg-astra-surface p-4 space-y-3">
              <h4 className="text-xs font-bold text-blue-400">New Connector</h4>
              <div className="grid grid-cols-2 gap-3">
                <div>
                  <label className="text-[10px] text-slate-500 mb-1 block">Designator *</label>
                  <input value={connForm.designator} onChange={e => setConnForm({...connForm, designator: e.target.value})} placeholder="J1" className="w-full rounded-lg border border-astra-border bg-astra-surface-alt px-3 py-2 text-sm text-slate-200 outline-none focus:border-blue-500/50" />
                </div>
                <div>
                  <label className="text-[10px] text-slate-500 mb-1 block">Name</label>
                  <input value={connForm.name} onChange={e => setConnForm({...connForm, name: e.target.value})} placeholder="Power/Data" className="w-full rounded-lg border border-astra-border bg-astra-surface-alt px-3 py-2 text-sm text-slate-200 outline-none focus:border-blue-500/50" />
                </div>
                <div>
                ...
                <div>
                  <label className="text-[10px] text-slate-500 mb-1 block">Total Contacts</label>
                  <input type="number" value={connForm.total_contacts} onChange={e => setConnForm({...connForm, total_contacts: parseInt(e.target.value) || 0})} className="w-full rounded-lg border border-astra-border bg-astra-surface-alt px-3 py-2 text-sm text-slate-200 outline-none focus:border-blue-500/50" />
                </div>
              </div>
              <div className="flex gap-2 pt-1">
                <button onClick={handleAddConnector} disabled={!connForm.designator || savingConn} className="flex items-center gap-1.5 rounded-lg bg-blue-500 px-4 py-2 text-xs font-semibold text-white hover:bg-blue-600 disabled:opacity-50">
                  {savingConn ? <Loader2 className="h-3.5 w-3.5 animate-spin" /> : <Save className="h-3.5 w-3.5" />} Create
                </button>
                <button onClick={() => setShowAddConnector(false)} className="rounded-lg border border-astra-border px-4 py-2 text-xs text-slate-400 hover:text-slate-200">Cancel</button>
              </div>
            </div>
          )}
          {unit.connectors.length === 0 ? (
            <div className="py-12 text-center text-sm text-slate-500">No connectors defined yet. Click "Add Connector" above to get started.</div>
      ...
      {tab === 'communication' && (
        <div className="space-y-4">
          <div className="flex items-center justify-between mb-2">
            <h3 className="text-xs font-semibold text-slate-400">{unit.bus_definitions.length} bus definition{unit.bus_definitions.length !== 1 ? 's' : ''}</h3>
            <button onClick={() => setShowAddBus(true)} className="flex items-center gap-1.5 rounded-lg bg-blue-500 px-3 py-2 text-xs font-semibold text-white hover:bg-blue-600">
              <Plus className="h-3.5 w-3.5" /> Add Bus
            </button>
          </div>
          {showAddBus && (
            <div className="rounded-xl border border-blue-500/30 bg-astra-surface p-4 space-y-3">
              <h4 className="text-xs font-bold text-blue-400">New Bus Definition</h4>
              <div className="grid grid-cols-3 gap-3">
                <div>
                  <label className="text-[10px] text-slate-500 mb-1 block">Name *</label>
                  <input value={busForm.name} onChange={e => setBusForm({...busForm, name: e.target.value})} placeholder="MIL-STD-1553B Bus A" className="w-full rounded-lg border border-astra-border bg-astra-surface-alt px-3 py-2 text-sm text-slate-200 outline-none focus:border-blue-500/50" />
                </div>
                <div>
                ...
                <div>
                  <label className="text-[10px] text-slate-500 mb-1 block">Protocol Version</label>
                  <input value={busForm.protocol_version} onChange={e => setBusForm({...busForm, protocol_version: e.target.value})} placeholder="e.g. Rev B" className="w-full rounded-lg border border-astra-border bg-astra-surface-alt px-3 py-2 text-sm text-slate-200 outline-none focus:border-blue-500/50" />
                </div>
              </div>
              <div className="flex gap-2 pt-1">
                <button onClick={handleAddBus} disabled={!busForm.name || savingBus} className="flex items-center gap-1.5 rounded-lg bg-blue-500 px-4 py-2 text-xs font-semibold text-white hover:bg-blue-600 disabled:opacity-50">
                  {savingBus ? <Loader2 className="h-3.5 w-3.5 animate-spin" /> : <Save className="h-3.5 w-3.5" />} Create
                </button>
                <button onClick={() => setShowAddBus(false)} className="rounded-lg border border-astra-border px-4 py-2 text-xs text-slate-400 hover:text-slate-200">Cancel</button>
              </div>
            </div>
          )}
          {unit.bus_definitions.length === 0 ? (
            <div className="py-12 text-center text-sm text-slate-500">No bus definitions yet. Click "Add Bus" above to get started.</div>
